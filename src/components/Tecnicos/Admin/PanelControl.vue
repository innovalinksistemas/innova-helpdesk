...
onMounted(async () => {
  ...
  if (!empresasError && empresasData) {
    empresas.value = empresasData.length;
  } else {
    console.error("Error al contar empresas:", empresasError);
  }
});
...
<template>
  <div class="">
    ...
    <div class="flex flex-row space-x-4 text-center">
      <div
        class="w-48 h-24 p-3 rounded-2xl shadow-xl backdrop-blur-xl bg-white/10 border border-white/30 text-white flex items-center justify-between cursor-pointer"
         @click="router.push({ name: 'AsignarTickets' })"
      >
        <img
      ...
      <div
        class="w-48 h-24 p-3 rounded-2xl shadow-xl backdrop-blur-xl bg-white/10 border border-white/30 text-white flex items-center justify-between"
      >
      ...
      <div
        class="w-48 h-24 p-3 rounded-2xl shadow-xl backdrop-blur-xl bg-white/10 border border-white/30 text-white flex items-center justify-between cursor-pointer hover:bg-white/20 transition"
        @click="router.push({ name: 'AdminClientes' })"

      >
        <img
          src="/src/assets/clientes.png"
          alt="clientes"
          class="w-9 h-8 mx-auto"
        />
        <div>
          <p>Clientes</p>
          <p>{{ empresas }}</p>
